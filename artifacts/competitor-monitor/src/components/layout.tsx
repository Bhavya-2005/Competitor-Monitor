...
function NotificationBell() {
  const { data: checks } = useListChecks({
    limit: 30,
  });
  ...
  const safeChecks = Array.isArray(checks)
    ? checks
    : Array.isArray((checks as any)?.data)
      ? (checks as any).data
      : [];

  const alerts = safeChecks
    .filter(
      (c: any) =>
        c?.hasChanges &&
        c?.status === "completed"
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        side="right"
        align="end"
        className="w-80 p-0 bg-background border-border/60"
      >
              ...
              {alerts.map((a: any) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    ...
                    <span
                      className={`text-xs font-medium capitalize flex-none ${
                        changeColors[a.changeType ?? ""] ??
